...
const MyApps = () => {
  ...
  const { myApps, loadMyApps, loadMyShareApps } = useAppStore();
  ...
  /* 加载模型 */
  const { isFetching } = useQuery(['loadApps'], () => loadMyShareApps(true), {
    refetchOnMount: true
  });
  ...
  return (
    <PageContainer isLoading={isFetching} insertProps={{ px: [5, '48px'] }}>
        ...
        {myApps.map((app) => (
          <MyTooltip key={app._id} label={t('app.To Chat')}>
            <Box
              ...
              onClick={() => {
                router.push(`/chat/app?shareId=${app.shareId}`);
              }}
            >
      ...
      <ConfirmModal />
      {isOpenCreateModal && (
        <CreateModal onClose={onCloseCreateModal} onSuccess={() => loadMyShareApps(true)} />
      )}
    </PageContainer>
  );
};
