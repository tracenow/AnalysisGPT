...
function EditLinkModal({
  ...
  const { data: teamList = [] } = useQuery(['getAllTeamList'], () => getAllTeamList());
  return (
    <MyModal
      isOpen={true}
    ...
    >
      <ModalBody>
        ...
        <Flex alignItems={'center'} mt={4}>
          ...
          <Select
            {...register('limit.shareTeam')}
            placeholder={t('outlink.choose share team')}
            defaultValue={defaultData?.limit?.shareTeam}
          >
            {teamList.map((item) => (
              <option key={item._id} value={item._id}>
                {item.name}
